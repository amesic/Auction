...
public class ProductController {
    ...
    public ResponseEntity<PaginationInfo<Product>> findPaginatedNewArrivals(
    ...
    public ResponseEntity<PaginationInfo<Product>> findPaginatedLastChance(
            @RequestParam("page") final Long page,
            @RequestParam("size") final Long size) {
    ...
    public ResponseEntity<Long> getNumberViewersOfProduct(@RequestParam("id") final Long id) {
        ...
        return new ResponseEntity<>(this.numberOfViewersByProduct.stream()
                .filter(pv -> pv.getProductId().equals(id)).findFirst().get()
                .getNumberOfCurrentViewers(),
                HttpStatus.OK);
    }
    ...
    public void onDisconnectMessage(final UserWatchProductId idEmail) {
        if (!idEmail.getEmail().equals("") && this.usersWatchingProduct.stream()
                .anyMatch(uw -> uw.getSessionId().equals(idEmail.getSessionId()))
        ) {
        ...
        this.usersWatchingProduct.forEach(uw -> {
            if (uw.getProductId().equals(idEmail.getProductId())) {
                ...
                headerAccessor.setSessionId(uw.getSessionId());
                headerAccessor.setLeaveMutable(true);
                Long numberOfViewers = this.numberOfViewersByProduct.stream()
                        .filter(pv -> pv.getProductId().equals(idEmail.getProductId())).findFirst()
                        .get().getNumberOfCurrentViewers();
                numberOfViewers--;
    ...
    public void onConnectMessage(final UserWatchProductId idEmail) {
        if (!idEmail.getEmail().equals("")) {
            if (this.usersWatchingProduct.stream()
                    .noneMatch(uw -> uw.getSessionId().equals(idEmail.getSessionId()))
            ) {
            ...
            if (result.isEmpty()) {
                ...
                this.usersWatchingProduct.forEach(uw -> {
                    if (uw.getProductId().equals(idEmail.getProductId())) {
                        ...
                        headerAccessor.setSessionId(uw.getSessionId());
                        headerAccessor.setLeaveMutable(true);
                        this.template.convertAndSendToUser(uw.getSessionId(),
                                "/queue/notify",
                                this.numberOfViewersByProduct.stream().filter(pv -> pv.getProductId()
                                        .equals(idEmail.getProductId())).findFirst()
                                        .get().getNumberOfCurrentViewers(), headerAccessor.getMessageHeaders());
                    }
                });
                this.numberOfViewersByProduct.stream()
    ...
    public ResponseEntity<PaginationInfo<Product>> getAllProductsBySort(
            @RequestParam final String typeOfSort,
    ...
    public ResponseEntity<PriceProductInfo> getAllProductsByPrice(
            ...
            @RequestParam(required = false) final String searchUser) {
        return new ResponseEntity<>(productService.getNumberProductsByPrice(
                subcategoryId,
                listOfCharacteristicsClicked,
                searchUser
        ),
                HttpStatus.OK);
    }
